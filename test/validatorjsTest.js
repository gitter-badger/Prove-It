...
describe('ValidatorJS', function () {
    ...
    it('should validate strings against an expected value', function () {
        test({ check: 'equals', args: ['abc'], valid: ['abc'], invalid: ['Abc', '123'] });
    });

    it('should validate strings contain another string', function () {
        test({ check: 'contains', args: ['foo'], valid: ['foo', 'foobar', 'bazfoo'],
            invalid: ['bar', 'fobar'] });
    });

    it('should validate strings against a pattern', function () {
        test({ check: 'matches', args: [/abc/], valid: ['abc', 'abcdef', '123abc'],
            invalid: ['acb', 'Abc'] });
        test({ check: 'matches', args: ['abc'], valid: ['abc', 'abcdef', '123abc'],
            invalid: ['acb', 'Abc'] });
        test({ check: 'matches', args: ['abc', 'i'], valid: ['abc', 'abcdef', '123abc', 'AbC'],
            invalid: ['acb'] });
    });

    it('should validate strings by length', function () {
        test({ check: 'isLength', args: [2], valid: ['abc', 'de', 'abcd'], invalid: [ '', 'a' ] });
        test({ check: 'isLength', args: [2, 3], valid: ['abc', 'de'], invalid: [ '', 'a', 'abcd' ] });
        test({ check: 'isLength', args: [2, 3],
            valid: ['干𩸽', '𠮷野家'],
            invalid: [
                '',
                '𠀋',
    ...
    it('should validate strings by byte length', function () {
        test({ check: 'isByteLength', args: [2], valid: ['abc', 'de', 'abcd'], invalid: [ '', 'a' ] });
        test({ check: 'isByteLength', args: [2, 3], valid: ['abc', 'de'], invalid: [ '', 'a', 'abcd' ] });
    });
    ...
    it('should validate a string that is in another string or array', function () {
        test({ check: 'isIn', args: ['foobar'], valid: ['foo', 'bar', 'foobar', ''],
            invalid: ['foobarbaz', 'barfoo'] });
        test({ check: 'isIn', args: [
            ['foo', 'bar']
        ], valid: ['foo', 'bar'],
        ...
        test({ check: 'isIn', args: [
            ...
            invalid: ['4', ''] });
        test({ check: 'isIn', valid: [], invalid: ['foo', ''] });
    });
    ...
    it('should validate dates against a start date', function () {
        test({ check: 'isAfter', args: ['2011-08-03'],
    ...
    it('should validate dates against an end date', function () {
        test({ check: 'isBefore', args: ['08/04/2011'],
            valid: [ '2010-07-02', '2010-08-04', new Date(0) ],
            invalid: [ '08/04/2011', new Date(2011, 9, 10) ] });
        test({ check: 'isBefore', args: [ new Date(2011, 7, 4) ],
            valid: [ '2010-07-02', '2010-08-04', new Date(0) ],
            invalid: [ '08/04/2011', new Date(2011, 9, 10) ] });
        test({ check: 'isBefore',
